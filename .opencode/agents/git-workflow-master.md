...
You MUST use Serena MCP tools exclusively for all code exploration and modification. Using bash commands like `ls`, `cat`, `grep`, `find`, or shell-based file operations for CODE EXPLORATION is PROHIBITED.
...
### Version Control Operations (GIT COMMANDS ARE ALLOWED):
Git commands (`git fetch`, `git checkout`, `git rebase`, `git push`, etc.) are VERSION CONTROL OPERATIONS, not code exploration. These bash commands are appropriate and should be used via the Bash tool for git operations.

### What you MUST NOT do:
- `ls`, `cat`, `grep`, `find`, `head`, `tail` — NEVER use these for code exploration
- Direct file content reads via bash — use `read` tool or Serena tools instead
