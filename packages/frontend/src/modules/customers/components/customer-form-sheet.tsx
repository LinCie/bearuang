...
import type { Customer } from 'backend/src/modules/customers/customers.route'
...
export function CustomerFormSheet({
  ...
  React.useEffect(() => {
    if (open) {
      ...
      form.setFieldValue('isActive', customer?.isActive ?? true)
      setServerError(null)
    }
  }, [open, customer])

  const title = isEditing ? 'Edit Pelanggan' : 'Pelanggan Baru'
  const description = isEditing
    ? 'Perbarui informasi pelanggan untuk memastikan data tetap akurat.'
    : 'Tambahkan pelanggan baru untuk mengelola hubungan bisnis dengan klien Anda.'
  const submitLabel = isEditing ? 'Simpan Perubahan' : 'Tambah Pelanggan'
      ...
      <SheetContent side="right" className="sm:max-w-md">
        ...
        >
          ...
          <form.Field
            name="name"
            validators={{
              onBlur: customerSchema.shape.name,
              onSubmit: customerSchema.shape.name,
            }}
          >
            {(field) => (
              <div className="space-y-1.5">
                ...
                <Input
                  id={field.name}
                  placeholder="Contoh: PT Customer Sejahtera"
          ...
          <form.Field
            name="email"
            validators={{
              onBlur: customerSchema.shape.email,
              onSubmit: customerSchema.shape.email,
            }}
          >
            {(field) => (
              <div className="space-y-1.5">
          ...
          <form.Field
            name="phone"
            validators={{
              onBlur: customerSchema.shape.phone,
              onSubmit: customerSchema.shape.phone,
            }}
          >
            {(field) => (
              <div className="space-y-1.5">
